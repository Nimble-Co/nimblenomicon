...
function searchPageUrlFromParams(params: URLSearchParams): string {
	const query = params.toString();
	return `${window.location.pathname}${query ? `?${query}` : ''}${window.location.hash}`;
}

function currentSearchPageUrl(): string {
...
export function stripInvalidTypeFromUrl(): void {
	...
	params.delete('type');
	window.history.replaceState({}, '', searchPageUrlFromParams(params));
}
...
export function setSearchPageUrl(
	...
	const next = searchPageUrlFromParams(params);
	if (next !== currentSearchPageUrl()) {
