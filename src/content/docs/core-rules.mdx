...
{skills.map((skill) => (
  <div
    set:html={renderMarkdown(
      `**${skill.name}** (${skill.stat}) ${skill.body}`,
    )}
  />
))}
...
{backgrounds.map((segment) => (
  <div
    set:html={renderMarkdown(`**${segment.name}** ${segment.body}`)}
  />
))}
...
{adventuringMotivations.map((motivation) => (
  <p
    set:html={renderMarkdown(
      `**${motivation.name}** ${motivation.body}`,
    )}
  />
))}
...
<ul>
  {weaponProperties.map((weaponProperty) => (
    <li
      set:html={renderMarkdown(
        `**${listLabel(weaponProperty.name)}** ${weaponProperty.body}`,
      )}
    />
  ))}
</ul>
...
<ul>
  {spellSchools.map((school) => (
    <li>
      <strong>{listLabel(`${school.name} Spells`)}</strong> {school.body}
    </li>
  ))}
</ul>
...
{optionalVariantRules.map((rule) => (
  <>
    <h3>{rule.name}</h3>
    <div set:html={renderMarkdown(rule.body)} />
  </>
))}
