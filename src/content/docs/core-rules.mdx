...
{skills.map((skill) => (<>
  ...
  {skill.callout && <blockquote dangerouslySetInnerHTML={{ __html: skill.callout }} />}
  </>
))}
...
<ul>
  {weaponProperties.map((weaponProperty) => (
    <>
      <li>
        <strong>{weaponProperty.name}.</strong> {weaponProperty.body}
      </li>
      {weaponProperty.callout && (
        <blockquote set:html={weaponProperty.callout}></blockquote>
      )}
    </>
  ))}
</ul>
    ...
    {weapons
      ...
      .map((w) => (
        <tr>
          <td>{w.item}</td>
          <td>{w.damage}</td>
          <td>{w.properties.map((p) => p.text).join(", ")}</td>
          <td>{w.cost}</td>
        </tr>
      ))}
  </tbody>
