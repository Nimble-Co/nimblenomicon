---
title: Core Rules
description: The core rules of Nimble RPG.
hidePageTitle: true
bannerInHero: true
---
...
import TermLineList from '../../components/TermLineList.astro';
import TermParagraphList from '../../components/TermParagraphList.astro';
import {
	adventuringMotivationParagraphLines,
	ancestryBlocks,
	armorTableSectionsData,
	backgroundsBlockLines,
	characterClassBulletLines,
	conditionBulletLines,
	coreRulesWeaponsTable,
	dcExampleBulletLines,
	downtimeBulletLines,
	glossaryParagraphLines,
	languageBulletLines,
	magicalItemBlocks,
	magicalItemRaritiesTable,
	miscAdventuringEquipmentTable,
	optionalVariantRuleSections,
	saveTypeBulletLines,
	sizeBulletLines,
	skillsParagraphLines,
	spellSchoolBulletLines,
	spellScrollCostsTable,
	statArrayBulletLines,
	statsParagraphLines,
	wandCostsTable,
	weaponPropertyBulletLines,
...
Heroes have 4 stats that impact their effectiveness in various tasks: 2 Key <Reference term="Stats" /> crucial to their class and 2 Secondary Stats. The maximum a hero's stat can typically go is +5 .

> **<Reference term="KEY" />.** When an ability or spell references “KEY,” use one of your Key <Reference term="Stats" />. If a stat is listed before a die roll, roll a number of dice equal to the stat. For example, “WIL d8” means that if your WIL is 2, you roll 2d8 (two eight-sided dice).

<TermParagraphList items={statsParagraphLines()} />
...
<TermParagraphList items={skillsParagraphLines()} />
...
<TermLineList items={dcExampleBulletLines()} />
...
<TermLineList items={saveTypeBulletLines()} />

### Heroes and Saves

- Unless otherwise noted, the DC for effects a hero causes is **10+<Reference term="KEY" />**.
- When making saves, each hero has 1 advantaged save (+), 1 disadvantaged save (-), and 2 neutral saves.

> **Example.** A <Reference term="Berserker" /> (STR+, INT-) would roll all of his STR saves with advantage and all of his INT saves with disadvantage.

## Size

Some spells and abilities affect differently sized objects or creatures. For size, use the following guidelines:

<TermLineList items={sizeBulletLines()} />
...
> **Example 1.** <Reference term="Greataxe" /> (2d6) with advantage. Roll 1 additional die and remove the lowest. You hit!

> **Example 2.** <Reference term="Greataxe" /> (2d6) with disadvantage 2. Roll 2 additional dice, remove the 2 highest. If there is a tie, always remove dice from left to right. You miss!

## Hit Points & Dying

Hit Points (HP) represent your ability to endure damage. Damage reduces your HP (which can't go below 0). When reduced to 0 HP, gain 1 <Reference term="Wound" />; you also gain the **<Reference term="Dying" />** condition until you regain HP. While Dying, actions are limited to 1, <Reference term="Concentration" /> is broken, and you are at risk of further serious harm:

- Attacking/casting spells causes **1** <Reference term="Wound" /> unless you make a **DC 10 STR save**.
- Taking damage while <Reference term="Dying" /> causes **2** Wounds; a crit causes **3** instead.
...
Some abilities or effects may grant temporary HP (temp HP); these are reduced first when taking damage. <Reference term="Temp HP" /> do not combine: If a hero has temp HP and would gain more, they instead choose which amount to keep. They expire after a Safe Rest.
...
> **Gritty <Reference term="Dying" /> Rules.** Optional rules for a more challenging or “old school” style of game: You can reduce the max number of Wounds heroes have. Any- where from a max of 5 Wounds for slightly harder, all the way down to 1–2 Wounds for a very lethal game!
>
> You can also have each <Reference term="Wound" /> impose a cumulative –1 penalty to rolls: skill checks, saves, or even ALL rolls! (If you're a real glutton for punishment.)
...
When a character is forcibly moved but stopped by an obstacle, they take **1d6** bludgeoning damage for every space this movement is shortened. If they hit another creature, both creatures split this damage. <Reference term="Falling" /> inflicts **1d6** bludgeoning damage for every 10 ft. fallen (2 spaces).
...
- larger than you, you gain the <Reference term="Riding" /> condition.
...
> **<Reference term="Restrained" />.** Functions like Grappled, but is caused by objects (e.g., chains, rope, roots) and ignores size restrictions. It can also be ended through any logical means, such as picking a lock or cutting/burning rope.
...
<TermLineList items={conditionBulletLines()} />

> **Other Minor Statuses.** Minor statuses (Smoldering, Charged, <Reference term="Distracted" />, etc.) do nothing on their own and end whenever combat does. Some spells and abilities have additional effects against such targets.
...
> **A Note on <Reference term="Surprise" />.** Merely being hidden or punching first is not sufficient to gain Surprise! If an enemy is on guard or at all aware of your presence, they cannot be surprised.
...
A **<Reference term="Turn" />** is when 1 individual hero or monster group acts. A typical turn takes roughly 6 seconds of “in-world“ time. A **<Reference term="Round" />** is when all players and monsters have taken a turn acting. An **<Reference term="Encounter" />** is all of the rounds in a particular combat.

> **1/<Reference term="Turn" /> Abilities.** If you perform one on your turn, and can find a way to perform it on another creature's turn, you can do it again (e.g., Sneak <Reference term="Attack" /> combined with Opportunity Attack or Coordinated Strike!).
>
> **1/<Reference term="Round" /> Abilities.** These reset whenever your own turn begins (e.g., when you Defend, you cannot do so again until your turn comes back around).

### Acting Over Multiple Turns

For activities that cost more than 1 action, your actions can be spent over multiple turns in combat as long as <Reference term="Concentration" /> is maintained and you do not perform other actions or reactions in the meantime (unless they are free).

> **Example.** Book, the <Reference term="Mage" />, wants to cast <Reference term="Glacier Strike" /> (3 actions), but only has 1 action left. He'll spend 1 action on this turn to start casting the spell, and on his next turn spend 2 more to finish casting it. He's taking a risk, though: he can't Defend himself and may need to rely on his party to protect him.
...
After a Safe Rest, heroes recover all of their HP, Hit Dice, mana (and other class-specific resources), and heal 1 <Reference term="Wound" />. Safe Rests are a great opportunity for downtime activities as well.
...
<TermLineList items={downtimeBulletLines()} />

# Character Creation

Your hero is how you will make your mark on the world. Here's how you can make your own hero:

### Step 1: Choose Your [Class](/classes/)

This will have the largest impact on the other character choices you make and how you interact with the world:

<TermLineList items={characterClassBulletLines()} />

### Step 2: Choose Your [Ancestry](/core-rules/#common-ancestries) & [Background](/core-rules/#backgrounds)

(and optionally, choose your [adventuring motivation](/core-rules/#adventuring-motivation)).
...
Fill in your character details: Name, Ancestry, Class, Level, Height, and Weight. Fill in the arrow for your advantaged (+) and disadvantaged (-) Saves, and choose a stat array (recommended: place the highest numbers in the <Reference term="KEY" /> stats for your class).

<TermLineList items={statArrayBulletLines()} />

> **Example.** <Reference term="The Cheat" /> has DEX and INT as their <Reference term="KEY" /> stats. With the Min-Max array, you might put +3 in DEX, +1 in INT, and -1 in WIL and STR. You'd also mark ▲ for DEX saves and ▼ for WIL saves.
...
All heroes speak <Reference term="Common" /> by default. Each point of INT grants you an additional language known. These languages are typically spoken by:

<TermLineList items={languageBulletLines()} />
...
- Small, related items can be grouped into one slot for simplicity (e.g., <Reference term="Camping Supplies" />: soap, blanket, brush, rations).
...
<TermParagraphList as="div" items={backgroundsBlockLines()} />
...
<TermParagraphList items={adventuringMotivationParagraphLines()} />
...
{
armorTableSectionsData().map(({ sectionLabel, columns, rows }) => (
...
<TermLineList items={weaponPropertyBulletLines()} />
...
Heroes may wield 2 <Reference term="Light" /> weapons at the same time. While dual wielding, you may gain advantage on an attack with those weapons, 1/round.

You may dual wield 1-handed weapons without the <Reference term="Light" /> property if your STR is 3 or greater, or 1 weapon without the Light property if your STR is 2. If completely unarmed, fists/feet can be considered dual wielded.
...
Can you reflavor a staff as a greatclub? Sure! A sai instead of a dagger? Of course! Could you pay a blacksmith to make your 1d12 <Reference term="Greatmaul" /> deal 2d6 or 3d4 damage instead? Since the dice all add up to 12, it won't break the game; ask your GM! Can a blacksmith make your _Dagger_ deal that much damage? No, sorry!

### Melee Weapons

<DataTable {...coreRulesWeaponsTable('melee')} />

### Ranged Weapons

<DataTable {...coreRulesWeaponsTable('ranged')} />
...
Heroes cannot typically see in the dark. Fighting in the dark imposes the <Reference term="Blinded" /> condition. Heroes can bring a lantern or torch to light their way in the dark as long as they have a hand free (or some other way) to hold it. A torch typically lasts for 1 dungeon (or 1 level of a very large one). A lamp with oil can typically last for 1 entire outing, until your next Safe Rest. Torches and lanterns cast light up to 6 spaces away; beyond this is darkness.
...
<DataTable {...miscAdventuringEquipmentTable()} />
...
<DataTable {...magicalItemRaritiesTable()} />

<HeadingBlockList
	blocks={magicalItemBlocks('standard', { source: 'core-rules' })}
/>
...
<DataTable {...spellScrollCostsTable()} />
...
<DataTable {...wandCostsTable()} />

<HeadingBlockList
	blocks={magicalItemBlocks('wand', { source: 'core-rules' })}
/>
...
**<Reference term="Mana" />.** Powerful tiered spells require mana—a portion of the spellcaster's life force—to fuel them. A spell's mana cost is equal to its tier. See the _Heroes_ book for more info on how different classes gain mana. Cantrips, cost no mana.
...
**Multi-<Reference term="Target" />/Area of Effect (AoE).** Multi-target/AoE attacks follow special rules to streamline play: they do not miss or crit, and a single roll is applied to all targets. (Ignore (dis)advantage from conditions on individual targets—such as <Reference term="Blinded" /> or Grappled—though anything granting you (dis)advantage still applies to your roll as normal.)

> **Save Spells.** A hero's save DC is typically 10+<Reference term="KEY" />.
>
...
<TermLineList items={spellSchoolBulletLines()} />
...
<SpellList spellSchoolId="fire" />

## Ice Spells

<SpellList spellSchoolId="ice" />

## Lightning Spells

<SpellList spellSchoolId="lightning" />

## Wind Spells

<SpellList spellSchoolId="wind" />

## Radiant Spells

<SpellList spellSchoolId="radiant" />

## Necrotic Spells

<SpellList spellSchoolId="necrotic" />
...
<SpellList spellSchoolId="fire" utility />

### Ice

<SpellList spellSchoolId="ice" utility />

### Lightning

<SpellList spellSchoolId="lightning" utility />

### Wind

<SpellList spellSchoolId="wind" utility />

### Radiant

<SpellList spellSchoolId="radiant" utility />

### Necrotic

<SpellList spellSchoolId="necrotic" utility />
...
<HeadingBlockList blocks={optionalVariantRuleSections()} />
...
<TermParagraphList items={glossaryParagraphLines()} />
