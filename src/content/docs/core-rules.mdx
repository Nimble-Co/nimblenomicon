...
<ul>
  {saveTypes.map((saveType) => (
    <li>
      <strong>{listLabel(saveType.name)}</strong> {saveType.body}
    </li>
  ))}
</ul>
...
<ul>
  {conditions.map((condition) => (
    <li>
      <strong>{listLabel(condition.name)}</strong> {condition.body}
    </li>
  ))}
</ul>
...
<ul>
  {downtimeActivities.map((activity) => (
    <li>
      <strong>{listLabel(activity.name)}</strong> {activity.body}
    </li>
  ))}
</ul>
...
<ul>
  {characterClasses.map((heroClass) => (
    <li>
      <strong>{listLabel(heroClass.name)}</strong> {heroClass.body}
    </li>
  ))}
</ul>
...
<ul>
  {statArrays.map((arr) => (
    <li>
      <strong>{listLabel(arr.name, ":")}</strong> {arr.body}
    </li>
  ))}
</ul>
...
<ul>
  {languages.map((language) => (
    <li>
      <strong>{listLabel(language.name)}</strong> {language.body}
    </li>
  ))}
</ul>
...
{backgrounds.map((segment) => (
  <div set:html={renderMarkdown(segment.body)} />
))}
...
<ul>
  {weaponProperties.map((weaponProperty) => (
    <>
      <li>
        <strong>{listLabel(weaponProperty.name)}</strong> {weaponProperty.body}
      </li>
...
<ul>
  {spellSchools.map((school) => (
    <li>
      <strong>{listLabel(school.name)}</strong> {school.body}
    </li>
  ))}
</ul>
