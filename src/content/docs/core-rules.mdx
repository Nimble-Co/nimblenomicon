...
<ul>
  {saveTypes.map((saveType) => (
    <li>
      <strong>{saveType.name}</strong> {saveType.body}
    </li>
  ))}
</ul>
...
<ul>
  {downtimeActivities.map((activity) => (
    <li>
      <strong>{activity.name}</strong> {activity.body}
    </li>
  ))}
</ul>
...
<ul>
  {characterClasses.map((heroClass) => (
    <li>
      <strong>{heroClass.name}</strong> {heroClass.body}
    </li>
  ))}
</ul>
...
<ul>
  {statArrays.map((arr) => (
    <li>
      <strong>{arr.name}</strong> {arr.body}
    </li>
  ))}
</ul>
...
<ul>
  {languages.map((language) => (
    <li>
      <strong>{language.name}</strong> {language.body}
    </li>
  ))}
</ul>
...
<ul>
  {spellSchools.map((school) => (
    <li>
      <strong>{school.name}</strong> {school.body}
    </li>
  ))}
</ul>
...
{optionalVariantRules.map((rule) => (
  <>
    <h3>{rule.name}</h3>
    <p>{rule.body}</p>
    {rule.callout && <blockquote set:html={rule.callout} />}
  </>
))}
