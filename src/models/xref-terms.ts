...
/** Extra match strings that map to the same entry (optional per row in data later). */
function expandAliases(entries: XrefTermEntry[]): XrefTermEntry[] {
	const withAliases: XrefTermEntry[] = [];
	for (const e of entries) {
		withAliases.push(e);
		if (e.aliases?.length) {
			for (const alias of e.aliases) {
				if (alias.trim() === '') continue;
				withAliases.push({ ...e, term: alias.trim() });
			}
		}
	}
	return withAliases;
...
/** Sorted list of canonical entity rows — use for disambiguation and `Reference` (no alias-only rows). */
...
export function buildXrefTermList(): XrefTermEntry[] {
	const raw = expandAliases(buildEntries());
