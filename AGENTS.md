...
- The `site` option in `astro.config.ts` is set from **`CF_PAGES_URL`** on Cloudflare Pages builds, or **`SITE_URL`** when set elsewhere. When neither is present (typical local dev), `site` is omitted and you may see a harmless `@astrojs/sitemap` warning during builds. Ignore it.
...
- **Full-bleed hero callout:** Content inside `.nimble-full-bleed-hero__callout` is skipped (see `FullBleedHero.astro`); use manual links there if needed.
