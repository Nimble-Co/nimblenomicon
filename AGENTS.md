...
### SEO and social previews (Starlight + this site)

Starlight (via `@astrojs/starlight/utils/head`) already emits per-page **`<title>`**, **`<meta name="description">`**, **Open Graph** (`og:title`, `og:type`, `og:url` when `site` is set, `og:description`, `og:locale`, `og:site_name`), **Twitter** (`twitter:card` = `summary_large_image`), **canonical** and **sitemap** links when `site` is configured. It does **not** emit `og:image` / `twitter:image`; this project adds those in [`src/components/Head.astro`](src/components/Head.astro).

- **`astro.config.ts`:** `site` comes from `CF_PAGES_URL` or `SITE_URL` (required for absolute canonical URLs, sitemap, and social image URLs). A global **`description`** is set as the fallback when a page omits `description` in frontmatter.
- **Default share image:** [`public/og-default.png`](public/og-default.png) (1200×630). Regenerate with `npx tsx scripts/generate-og-default.ts` after changing the generator.
- **Docs MDX (`src/content/docs/*.mdx`):** Every page should have a compelling **`description`** in frontmatter (about **150–160 characters** — Starlight’s schema recommends this range). Optionally set **`shareImage`** to a repo image (e.g. `shareImage: ../../assets/manticor.webp`) so Open Graph uses that art instead of the default; hero pages already align `shareImage` with their banner art where applicable.
- **Data detail routes** (spells, equipment, etc.): Meta descriptions are built in [`src/detail/detail-page-adapters.ts`](src/detail/detail-page-adapters.ts) using [`truncateMetaDescription`](src/utils/seo-meta.ts) so snippets stay within a sane length.

**Verify:** With `SITE_URL=https://your-origin` run `npm run build && npm run preview`, then view page source or use a [Rich Results](https://search.google.com/test/rich-results) / sharing debugger to confirm `og:image` and descriptions.

- **Search (Orama)** uses `public/orama-data-search.json`, generated by `npm run build:orama-index` (`prebuild` and `predev` run this automatically). To test built output behavior, use `npm run build && npm run preview`.
