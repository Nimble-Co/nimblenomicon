...
- **Search (Orama)** uses `public/orama-data-search.json`, generated by `npm run build:orama-index` (`prebuild` and `predev` run this automatically). A second index, `public/orama-books-search.json`, is built from post–auto-xref `dist/` HTML during `npm run build` only (gitignored like the game index). Until you run a full production build, book full-text hits are omitted in quick search and on `/search/` when merging with “All types”; `npm run dev` alone does not produce that file. To test built output behavior, use `npm run build && npm run preview`.
